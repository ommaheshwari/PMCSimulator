...
def motion(structure, top_voxels, faces, totalth, time_steps,db_steps, pref, petch, pdep,padsorb,etch_iteration, nsegments=3):
    T=650
    tcount=0
    ...
    for i in range(0,time_steps): #iterate over all time
        ...
        if may_hit.shape[1]>0:
            ...
            if i1.shape[0]>0:
                ...
                # print('pid',pid)
                # print('top_voxels[indices1,:]',top_voxels[indices1,:])
        ...
        # if (i+1) in ALL_STEPS:
        #     create_csv(structure,N_PARTICLES,i+1,decay_factor,T,tsige,DIR,doe_file,csventry)
        #     if FPLOT:
        #         plotting(structure,DIR+"training_tsige_"+str(tsige)+"_nump_"+str(N_PARTICLES)+"_steps_"+str(i+1)+'_Temp_'+str(T)+'_decay_'+str(decay_factor),save=1,slice='h',angle=330)
    # print('Done')
